...
def send_bilibili_episode_notification(
    bot_token: str,
    chat_id: str,
    creator_name: str,
    episode: PodcastEpisode,
    ...
    """
    Build formatted notification card for a newly synced Bilibili episode and send via Telegram.
    """
    if not bot_token or not chat_id:
        return None

    safe_creator = html.escape(creator_name or "Bilibili UP主")
    safe_title = html.escape(episode.title or "新单集")
    duration_str = format_duration(episode.duration_seconds)
    pub_str = (
        episode.pub_date.strftime("%Y-%m-%d %H:%M:%S")
    ...
    lines = [
        "📺 <b>【Bilibili 播客有新单集更新】</b>",
        "",
        f"👤 <b>UP主</b>: {safe_creator}",
        f"🎬 <b>单集</b>: {safe_title}",
        f"⏱️ <b>时长</b>: {duration_str}",
        f"📅 <b>发布时间</b>: {pub_str}",
        "",
    ...
    if feed_url:
        lines.append(f"📻 <b>播客订阅源</b>: <code>{feed_url}</code>")
    if episode.webpage_url:
        lines.append(f"🔗 <b>B站原视频</b>: <a href=\"{episode.webpage_url}\">点击查看</a>")
